use f2rs_parser_combinator::provided::common::chained;
...
pub fn operation<S: TextSource>() -> impl Parser<S, Token = Expression<S::Span>> {
    chained(spaced(expression_monome()), spaced(operator())).map(
        |(mut first, mut tail): (
            Expression<S::Span>,
            Vec<(StringMatch<S::Span>, Expression<S::Span>)>,
        )| {
            let operator_priority = |op: &StringMatch<S::Span>| -> usize {
                OPERATORS
                    .iter()
                    .cloned()
                    .find(|(o, _)| *o == op.value)
            ...
            fn pick_ith<'a, Span>(
                ...
                tail: &'a mut Vec<(StringMatch<Span>, Expression<Span>)>,
                i: usize,
            ) -> (&'a mut Expression<Span>, StringMatch<Span>, Expression<Span>) {
