...
pub enum Statement<Span> {
    ...
    SpecialFunction(SpecialStatementFunction<Span>),
...
pub fn statement<S: TextSource>() -> impl Parser<S, Token = Statement<S::Span>> {
    alt! {
        ...
        call_statement(),
        special_statement_function().map(Statement::SpecialFunction),
        (spaced(keyword("continue")), eol_or_comment()).map(|(_, _)| Statement::Continue),
