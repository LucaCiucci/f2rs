...
#[derive(Debug, Clone)]
pub struct ScalarConstant<Span>(pub Constant<Span>);

#[syntax_rule(
    F18V007r1 rule "scalar-constant",
)]
pub fn scalar_constant<'a, S: TextSource + 'a>(cfg: &'a Cfg) -> impl Parser<S, Token = ScalarConstant<S::Span>> + 'a {
    constant(cfg).map(ScalarConstant)
}
...
#[syntax_rule(
    F18V007r1 rule "named-constant" #606,
)]
...
#[syntax_rule(
    F18V007r1 rule "int-constant" #607,
)]
